...
# ************************************************************************
calc_daily_solar <- function(lat, n, elev=0, y=0, sw_in, tc=23.0) {
	...
	# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	# 07. Calculate daily extraterrestrial radiation (ra_d), MJ/m^2
	# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	r_in<-sw_in*86400
	# error from dailymet radiation data, orginal estimations where made for flat surfaces
	if (is.na(sw_in)||ra_d==0||ra_d<r_in){
	  tau <- tau_o
	}else{
	  tau<-r_in/(ra_d)
	}
	...
	# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	ppfd_d <- (1e-6)*(kfFEC*(1 - kalb_vis)*r_in)
	solar$ppfd_mol.m2 <- ppfd_d
	
	# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	# 10. Estimate net longwave radiation (rnl), W/m^2
	# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	rnl <- (kb + (1 - kb)*tau)*(kA - 0.3*tc)
	# rnl<-(5.31e-13*(tc+273.15)^6-5.67e-8*(tc+273.15)^4)*0.863-90.491*tau+87.219 # Jiang, 2015
	# rnl <- (kb + (1 - kb)*tau)*((5.67e-8*(tc-273.15)^4)*(1-1.19)+171) #Linacre, 1968 eqn 20, 21
	# ea<-(1 − 0.84*(1-tau))*0.9 + 0.84*(1-tau)
	# rnl <- ((ea*5.67e-8*(tc-273.15)^4)*(1-1.19)+171)
	...
	if(sw_in==0||is.na(sw_in)){
		rw <- (1 - alb)*tau*kGsc*dr}
	else{
		rw <- (1 - alb)*(r_in)/((86400/pi)*(ru*pir*hs + rv*dsin(hs)))
	}
	...
	# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	# 13. Calculate daytime net radiation (rn_d), MJ/m^2
	# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	rn_d <- (86400/pi)*(hn*pir*(rw*ru - rnl) + rw*rv*dsin(hn))
	solar$rn_j.m2 <- rn_d
	
	# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	# 14. Calculate nighttime net radiation (rnn_d), MJ/m^2
	# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	# fixed iss#13
	rnn_d <- (86400/pi)*(
